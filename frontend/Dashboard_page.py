...
def _smart_forecast(ts_train, steps=28):
    """
    forecast: SARIMA for 6+ weeks of data, 
    Weighted Seasonal MA fallback for shorter windows.
    Preserves weekly ups/downs, no flat lines.
    """
    n = len(ts_train)
    ...
    def _seasonal_ma(series, steps):
        """Project forward using last 2 weeks as a repeating pattern + trend."""
        week = 7
        ...
        pattern = 0.35 * w1 + 0.65 * w2  # blend, more weight to recent

        # Gentle trend from last 14 points
        x = np.arange(len(last2))
        slope = np.polyfit(x, last2, 1)[0]
        slope = np.clip(slope, -2.0, 2.0)

        out = []
        for i in range(steps):
            out.append(max(0.0, float(pattern[i % week]) + slope * (i + 1)))
        return np.array(out)

    #  METHOD 1: 6+ weeks: SARIMA (raw, no smoothing) 
    if n >= 42:
        try:
            ...
            # Only clip extreme outliers,preserve normal weekly swings
            raw = raw.clip(lower=max(0, t_mean - 3 * t_std),
                           upper=t_mean + 3 * t_std)
            raw_vals = raw.values.astype(float)

            # Check if SARIMA produced a suspiciously flat line
            # (std of forecast < 20% of std of training data)
            if raw_vals.std() < t_std * 0.20:
                # SARIMA collapsed to mean, use seasonal MA instead
                print("[SARIMA] collapsed to flat line, using Seasonal MA")
                return _seasonal_ma(vals, steps), "Seasonal MA"

            return raw_vals, "SARIMA"

        except Exception as e:
            print(f"[SARIMA failed] {e} - falling back")

    #  METHOD 2: short data : Seasonal MA 
    if n >= 14:
        return _seasonal_ma(vals, steps), "Seasonal MA"

    #  METHOD 3: very short : flat mean 
...
class DashboardPage(BasePage):
    ...
    def _build_header(self):
        ...
        tk.Label(left, text="Sales Forecasting",
                 font=("Georgia", 20, "bold"), fg=FG_PRIMARY, bg=DARK_BG).pack(anchor="w")

        right = tk.Frame(hdr, bg=DARK_BG)
        right.pack(side="right")
        self.status_dot = tk.Label(right, text="●", font=("Helvetica", 13),
        ...
        tk.Label(right, textvariable=self.status_var,
                 font=("Helvetica", 9, "italic"), fg=FG_MUTED, bg=DARK_BG).pack(side="right")

        tk.Frame(self.content, bg="#222222", height=1).pack(fill="x", padx=30, pady=4)

    def _build_controls(self):
        bar = tk.Frame(self.content, bg=PANEL_BG)
        bar.pack(fill="x", padx=30, pady=(0, 6), ipady=9)

        up_btn = tk.Button(bar, text="Upload CSV", command=self.load_csv,
                           font=("Helvetica", 9, "bold"),
                           bg=ACCENT_PINK, fg=FG_PRIMARY,
                           padx=18, pady=7, bd=0, relief="flat", cursor="hand2")
        up_btn.pack(side="left", padx=(14, 8))
        _hover(up_btn, ACCENT_PINK, "#D0005A")

        tk.Label(bar, text="Training period:", font=("Helvetica", 9),
        ...
        tk.Label(bar, text="weeks", font=("Helvetica", 9),
                 fg=FG_MUTED, bg=PANEL_BG).pack(side="left", padx=(2, 14))

        upd_btn = tk.Button(bar, text="Update Forecast",
                            command=self.update_forecast,
                            font=("Helvetica", 9), bg=CARD_BG, fg=FG_SECONDARY,
                            padx=14, pady=7, bd=0, relief="flat", cursor="hand2")
        upd_btn.pack(side="left", padx=4)
        _hover(upd_btn, CARD_BG, "#2A2A2A")

        tk.Label(bar, text="Zoom:", font=("Helvetica", 9),
        ...
        zm = ttk.Combobox(bar, textvariable=self.zoom_var,
                          values=["All", "Last 7 days", "Last 14 days",
                                  "Last 30 days", "Forecast only"],
                          width=13, state="readonly", font=("Helvetica", 9))
        zm.pack(side="left")
        zm.bind("<<ComboboxSelected>>", lambda e: self.apply_zoom())

        # Method label: shows which algorithm is active
        self.method_var = tk.StringVar(value="")
        tk.Label(bar, textvariable=self.method_var,
                 font=("Helvetica", 8, "italic"), fg=ACCENT_BLUE,
                 bg=PANEL_BG).pack(side="left", padx=(16, 0))
    ...
    def _build_main_area(self):
        ...
        tk.Label(title_row, text="SARIMA Sales Forecast",
                 font=("Georgia", 12, "bold"), fg=FG_PRIMARY, bg=CARD_BG).pack(side="left")
        tk.Label(title_row,
                 text=" Solid (-) = historical  Dashed (•) = predicted",
                 font=("Helvetica", 8), fg=FG_MUTED, bg=CARD_BG).pack(side="left")
        ...
        tk.Label(lh, text="Historical Sales Data",
                 font=("Helvetica", 9, "bold"), fg=FG_PRIMARY,
                 bg=PANEL_BG, pady=7, padx=10).pack(side="left")
        tk.Label(lh, text="(first 100 rows)",
                 font=("Helvetica", 8), fg=FG_MUTED, bg=PANEL_BG).pack(side="left")
        ...
        self.tree_actual = ttk.Treeview(li, show="headings", style="Pink.Treeview",
        ...
        tk.Label(rh, text="4-Week Forecast",
                 font=("Helvetica", 9, "bold"), fg=ACCENT_GREEN,
                 bg=PANEL_BG, pady=7, padx=10).pack(side="left")
        ...
        self.tree_forecast = ttk.Treeview(ri, show="headings", style="Pink.Treeview",
        ...
        tk.Label(tables_frame,
                 ...
                 font=("Helvetica", 8, "italic"), fg=FG_MUTED, bg=DARK_BG).pack(
    ...
    def update_forecast(self):
        if self.data is not None:
            self._set_status("Updating...", ACCENT_AMB)
            self.content.update()
            self.run_forecast()
        else:
    ...
    def run_forecast(self):
        ...
        for ci, col in enumerate([c for c in self.data.columns if c != "Date"]):
            try:
                full_ts = (
                ...
                # Get forecast using smart method selector
                smooth_vals, method = _smart_forecast(ts_train, steps=FORECAST_DAYS)
                methods_used.add(method)

                # Compute std for confidence band
                t_std = float(ts_train.std())
    ...
    def apply_zoom(self):
        ...
        all_dates = []
        for d in self._plot_data.values():
            all_dates.extend(d["ts"].index.tolist())
            all_dates.extend(d["future_dates"].tolist())
        if not all_dates:
            return
        max_d = max(all_dates)
        min_d = min(all_dates)
        ...
        if zoom == "Last 7 days":
            self.ax.set_xlim(max_d - pd.Timedelta(days=7), max_d)
        elif zoom == "Last 14 days":
            self.ax.set_xlim(max_d - pd.Timedelta(days=14), max_d)
        elif zoom == "Last 30 days":
            self.ax.set_xlim(max_d - pd.Timedelta(days=30), max_d)
        elif zoom == "Forecast only" and self._forecast_start:
            self.ax.set_xlim(
                self._forecast_start - pd.Timedelta(days=3), max_d)
        else:
    ...
    def _populate_actual_table(self, df):
        ...
        for _, row in display.iterrows():
            vals = list(row)
            if display.columns[0] == "Date" and isinstance(vals[0], pd.Timestamp):
                vals[0] = vals[0].strftime("%d %b %Y")
            self.tree_actual.insert("", "end", values=vals)

    def _populate_forecast_table(self, forecast_dict):
        """Display forecast as Week rows x Product columns - much easier to read."""
        self.tree_forecast.delete(*self.tree_forecast.get_children())

        if not forecast_dict:
            return

        # forecast_dict = {"Croissant - Wk 1": 62.1, "Americano - Wk 1": 96.0, ...}
        # Restructure into: {product: {wk: val}}
        products = []
        weeks = []
        data = {}
